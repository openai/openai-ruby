...
## Let a browser own a WebRTC conversation

For a browser-owned WebRTC session, authenticate the application user and
authorize their tenant before minting a short-lived delegated credential with
the generated client-secret resource:
...
# Return only secret.value and secret.expires_at to the authorized browser.
# The browser exchanges its SDP offer directly with OpenAI.
```

The browser owns its peer connection, microphone permission, audio playback,
data channel, SDP exchange, and teardown. Ruby never creates the call and does
not receive a call ID, so a browser disconnect during token delivery cannot
orphan a server-owned call. Session configuration attached to the secret can be
overridden by the browser, and one secret can create multiple sessions before
expiration; authenticate and authorize issuance independently rather than
treating the secret as immutable policy or a single-use grant.

The runnable
[`webrtc_conversation.rb`](examples/realtime/webrtc_conversation.rb) and its
[browser page](examples/realtime/webrtc_conversation.html) demonstrate
application-token authorization, loopback-only Host/Origin checks, CSRF
protection, per-user issuance limits, 60-second secret expiration, `no-store`
responses, safe diagnostics, and deterministic browser media teardown. See the
[Realtime examples README](examples/realtime/README.md#run-a-browser-owned-webrtc-conversation)
for setup and production authorization requirements.

## Create and control a server-owned WebRTC call

Create a WebRTC call from your server using the SDP offer generated by the
browser or other peer. The generated HTTP method returns an
`OpenAI::HTTPClient::Response`, preserving the SDP answer, response status,
request metadata, and the `Location` header containing the new call ID:
...
answer_sdp = response.body.to_a.join
call_location = URI.parse(response.headers.fetch("location"))
call_id = call_location.path.split("/").last

# Return answer_sdp to the browser to complete its WebRTC peer connection.
# Keep the API key and call-control authority on your server.
```

The response body is a one-shot stream: read it once and retain the resulting
SDP answer if your application needs it again. A response can omit `Location`;
in that case its SDP answer remains available, but the call cannot be attached
to through an ID from that response. Call creation does not retry by default,
preventing ambiguous network failures from allocating duplicate sessions. An
application that owns its retry policy can explicitly override it with
`request_options: {max_retries: 1}`.

Unlike browser-owned credential issuance, this flow allocates a server-owned
call. The application must explicitly retain ownership, authorize any
subsequent sideband attachment, and arrange hangup when answer delivery fails
after the complete response has been returned. A failure while the SDK is still
buffering a successful SDP response can occur before the call ID reaches the
application; this response-ownership limitation is unresolved and cannot be
repaired by application-level example code.
...
transport redacts it from optional WebSocket request-target and endpoint
traces, HTTP call paths and `Location` headers, and
`RealtimeConnectionError#url`. API keys and other sensitive headers remain
redacted. Sideband connections inherit the same
...
approval through the generic Realtime connection API, alongside browser-owned
WebRTC credential issuance, generated server-owned WebRTC/SDP call creation,
and sideband control of existing WebRTC and SIP calls.
The WebRTC browser example owns continuous microphone capture and full-duplex
audio inside the browser. Ruby-owned continuous capture, concurrent live
captioning, full-duplex WebSocket orchestration, SIP call-ownership workflows,
and translation connections remain out of scope. Existing generated HTTP
resources remain generated-code-owned.
