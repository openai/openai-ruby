# typed: strong

module OpenAI
  module Resources
    class FineTuning
      class Jobs
        # Manage fine-tuning jobs to tailor a model to your specific training data.
        class Checkpoints
          sig { returns(Checkpoints::WithRawResponse) }
          def with_raw_response
          end

          # List checkpoints for a fine-tuning job.
          sig do
            params(
              fine_tuning_job_id: String,
              after: String,
              limit: Integer,
              request_options: OpenAI::RequestOptions::OrHash
            ).returns(
              OpenAI::Internal::CursorPage[
                OpenAI::FineTuning::Jobs::FineTuningJobCheckpoint
              ]
            )
          end
          def list(
            # The ID of the fine-tuning job to get checkpoints for.
            fine_tuning_job_id,
            # Identifier for the last checkpoint ID from the previous pagination request.
            after: nil,
            # Number of checkpoints to retrieve.
            limit: nil,
            request_options: {}
          )
          end

          # @api private
          sig { params(client: OpenAI::Internal::Transport::RequestClient).returns(T.attached_class) }
          def self.new(client:)
          end

          class WithRawResponse
            sig { params(fine_tuning_job_id: String, after: String, limit: Integer, request_options: OpenAI::RequestOptions::OrHash).returns(OpenAI::RawResponse[OpenAI::Internal::CursorPage[OpenAI::FineTuning::Jobs::FineTuningJobCheckpoint]]) }
            def list(
              # The ID of the fine-tuning job to get checkpoints for.
              fine_tuning_job_id,
              # Identifier for the last checkpoint ID from the previous pagination request.
              after: nil,
              # Number of checkpoints to retrieve.
              limit: nil,
              request_options: {}
            )
            end

            # @api private
            sig { params(resource: Checkpoints).returns(T.attached_class) }
            def self.new(resource:)
            end
          end
        end
      end
    end
  end
end
