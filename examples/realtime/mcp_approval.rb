  ...
  module Examples
    module Realtime
      module MCPApproval
        ...
        def run(client:, model:, server_url:, prompt:, approval_policy: nil, output: $stdout)
        ...
        def initial_state
          {
            ...
            approval_request_arguments: nil,
            approval_submitted: false,
            tool_call_item_id: nil,
            tool_call_response_id: nil,
            tool_call_arguments: nil,
            tool_call_item: nil,
        ...
        def handle_event(connection, event, prompt:, state:, approval_policy:)
          case event
          ...
          when OpenAI::Realtime::ResponseMcpCallArgumentsDone
            ...
            state[:tool_call_arguments] = event.arguments
            submit_approval(connection, state: state)
        ...
        def handle_item(connection, item, prompt:, state:, approval_policy:)
          case item
          ...
          when OpenAI::Realtime::RealtimeMcpApprovalRequest
            ...
            unless authorized
              submit_approval_response(
                ...
                state: state,
                approved: false,
                reason: "Denied because no caller-provided application policy approved this request."
              )
              raise "Realtime MCP tool call was not approved by the application."
            end
        ...
        def validate_tool_call(state)
          ...
          unless item.approval_request_id == approval_request_id
            raise "Realtime MCP tool call did not match the approved request."
          end

          approved_arguments = state[:approval_request_arguments]
          generated_arguments = state[:tool_call_arguments]
          return unless approved_arguments && generated_arguments
          return if item.arguments == approved_arguments && item.arguments == generated_arguments

          raise "Realtime MCP tool call arguments did not match the approved request."
        end

        def submit_approval(connection, state:)
          return if state[:approval_submitted]
          return unless state[:approval_request_id] && state[:tool_call_arguments]

          unless state[:approval_request_arguments] == state[:tool_call_arguments]
            submit_approval_response(
              connection,
              state: state,
              approved: false,
              reason: "Denied because the generated arguments did not match the approved request."
            )
            raise "Realtime MCP tool call arguments did not match the approved request."
          end
          ...
          submit_approval_response(
            ...
            state: state,
            approved: true,
            reason: "Approved by the caller-provided application policy."
          )
        end

        def submit_approval_response(connection, state:, approved:, reason:)
          connection.conversation.items.create(
            type: :mcp_approval_response,
            id: "approval_response_1",
            approval_request_id: state.fetch(:approval_request_id),
            approve: approved,
            reason: reason
          )
        ...
        def exact_match_policy(allowed_tool_name:, allowed_arguments:)
          lambda do |server_label:, tool_name:, arguments:|
            server_label == SERVER_LABEL &&
...
if $PROGRAM_NAME == __FILE__
  approved_tool = ENV.fetch("MCP_APPROVED_TOOL")
  approved_arguments = ENV.fetch("MCP_APPROVED_ARGUMENTS")
  OpenAI::Examples::Realtime::MCPApproval.run_with_timeout(
    ...
    approval_policy: OpenAI::Examples::Realtime::MCPApproval.exact_match_policy(
      allowed_tool_name: approved_tool,
      allowed_arguments: approved_arguments
    )
  )
end
