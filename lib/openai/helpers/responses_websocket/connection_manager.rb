  ...
  module Responses
    ...
    class ConnectionManager
      ...
      def initialize(client:, websocket_base_url:, transport:, request_options:, transport_options:)
        ...
        reserved = transport_options.keys.select do |key|
          (key.is_a?(String) || key.is_a?(Symbol)) && RESERVED_TRANSPORT_OPTIONS.include?(key.to_sym)
        end

        unless reserved.empty?
          raise ArgumentError, "transport_options cannot include #{reserved.map(&:inspect).join(", ")}"
        end
      ...
      # @api private
      def open
        raise ArgumentError, "A block is required to open a Responses WebSocket." unless block_given?

        transport = @transport || OpenAI::Responses::Transports::AsyncWebSocket.new
        raise ArgumentError, "transport must respond to open" unless transport.respond_to?(:open)

        @client
          .with_responses_websocket_connection_request(
            ...
            transport
              .open(
                ...
                **@transport_options
                ...
                connection = OpenAI::Responses::Connection.new(socket: socket, url: request.fetch(:url))
                begin
                  yield(connection)
                ensure
                  pending_error = $ERROR_INFO
                  begin
                    if pending_error || connection.poisoned?
