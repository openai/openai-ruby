  ...
  module Models
    module Beta
      class AgentSessionTurnContentPartDoneEvent < OpenAI::Internal::Type::BaseModel
        ...
        #
        #   @return [Symbol, :"agent.session.turn.content_part.done"]
        required :type, const: :"agent.session.turn.content_part.done"

        # @!method initialize(content_index:, event_id:, item_id:, output_index:, part:, session_id:, turn_id:, type: :"agent.session.turn.content_part.done")
        ...
        #     The ID of the turn associated with the event, when applicable.
        #
        #   @param type [Symbol, :"agent.session.turn.content_part.done"]
        #     The type of the object. Always `agent.session.turn.content_part.done`.
      end
    end
  end
